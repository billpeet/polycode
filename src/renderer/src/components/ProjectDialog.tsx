...
type ConnectionMode = 'local' | 'ssh' | 'wsl'

function getInitialConnectionMode(project?: Project): ConnectionMode {
  if (project?.ssh?.host) return 'ssh'
  if (project?.wsl?.distro) return 'wsl'
  return 'local'
}
...
export default function ProjectDialog({ mode, project, onClose }: Props) {
  ...
  const [connectionMode, setConnectionMode] = useState<ConnectionMode>(
    getInitialConnectionMode(project)
  )
  // SSH fields
  const [sshHost, setSshHost] = useState(project?.ssh?.host ?? '')
  const [sshUser, setSshUser] = useState(project?.ssh?.user ?? '')
  const [sshPort, setSshPort] = useState(project?.ssh?.port?.toString() ?? '')
  const [sshKeyPath, setSshKeyPath] = useState(project?.ssh?.keyPath ?? '')
  // WSL fields
  const [wslDistro, setWslDistro] = useState(project?.wsl?.distro ?? '')
  ...
  const isWSL = connectionMode === 'wsl'
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      if (mode === 'edit' && project) {
        await updateProject(project.id, name.trim(), path.trim(), ssh, wsl)
      } else {
        await createProject(name.trim(), path.trim(), ssh, wsl)
      }
      onClose()
    } catch (err) {
      setError(String(err))
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-1">
              <button
                type="button"
                onClick={() => { setConnectionMode('local'); setTestResult(null); setError('') }}
                className="flex-1 rounded px-3 py-1.5 text-xs font-medium transition-colors"
                style={toggleStyle(connectionMode === 'local')}
              >
                Local
              </button>
              <button
                type="button"
                onClick={() => { setConnectionMode('ssh'); setTestResult(null); setError('') }}
                className="flex-1 rounded px-3 py-1.5 text-xs font-medium transition-colors"
                style={toggleStyle(connectionMode === 'ssh')}
              >
                SSH Remote
              </button>
              <button
                type="button"
                onClick={() => { setConnectionMode('wsl'); setTestResult(null); setError('') }}
                className="flex-1 rounded px-3 py-1.5 text-xs font-medium transition-colors"
                style={toggleStyle(connectionMode === 'wsl')}
              >
                WSL
              </button>
            </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {testing ? 'Testing...' : mode === 'edit' ? 'Save' : 'Create'}
            </button>
          </div>
        </form>
      </div>
