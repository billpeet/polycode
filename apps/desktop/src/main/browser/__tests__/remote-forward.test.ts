...
describe('remote browser forwarding', () => {
  it('loads the remote host localhost and preserves the browser Host header', async () => {
    ...
    const config = { enabled: true, host: '127.0.0.1', port: controlPort, token: 'test-token', webEnabled: false, allowedHostnames: [], tailscaleLogins: [] }
  ...
  it('rejects unauthenticated tunnels and non-loopback targets', async () => {
    ...
    attachRemoteBrowserTunnel(controlServer, {
      enabled: true, host: '127.0.0.1', port: controlPort, token: 'correct-token', webEnabled: false, allowedHostnames: [], tailscaleLogins: [],
    })
