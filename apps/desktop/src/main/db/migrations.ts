...
/**
 * Version 0 covers databases shipped before version tracking. These legacy
 * checks run once; later migrations must be ordered and schema-assumption based.
 */
function adoptLegacySchema(database: Database.Database): void {
  database.exec(`
  ...
  if (!hasModel) {
    database.exec(`ALTER TABLE threads ADD COLUMN model TEXT NOT NULL DEFAULT 'claude-opus-4-8'`)
  }
